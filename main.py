...
#about
def about():
    mess._show(title="About",message="This Attendance System is designed and developed  by Subrat Gupta")

#User Manual
def usermanual():
    mess._show(title="User Manual",message="To take attendence after the detection of face PRESS 'q' ")
...
def contact():
    mess._show(title='Contact us', message="Please contact on : guptasubrat111@gmail.com' ")
        ...
        with open("StudentDetails\StudentDetails.csv", 'r') as csvFile1:
            ...
            for row in reader1:
                if len(row) >= 3 and row[2] == Id:
                    mess._show(title='ID Already Exists', message='This ID is already registered.')
                    csvFile1.close()
                    return  # Exit the function if ID already exists
                
        # Continue with taking images and registering the user if ID is not found       
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite("TrainingImage\ " + name + "." + str(serial) + "." + Id + '.' + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + w])
                # display the frame
                cv2.putText(img, f"Images Taken: {sampleNum}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
def TrackImages():
    ...
    if exists1:
        ...
        cv2.destroyAllWindows()
        window.destroy()
   # attendance = None  # Initialize attendance variable before the loop
    while True:
        ret, im = cam.read()
...
window.title("Student  Attendance System by Subrat Gupta")
...
message3 = tk.Label(window, text="Student Attendance System using Machine Learning" ,fg="white",bg="#262523" ,width=55 ,height=1,font=('times', 29, ' bold '))
...
# add User Manual to menubar-------------------------------
menubar.add_command(label="User Manual",command=usermanual)
